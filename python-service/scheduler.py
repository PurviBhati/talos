...
def run_full_summarization():
    ...
    print("[run] Scheduler: Starting full summarization run...")

    try:
        channels = fetch_all_channels()
        print(f"[info] Found {len(channels)} channels to summarize")
        ...
        for ch in channels:
            try:
                ...
                if not messages:
                    print(f"[skip] [{ch['source']}] {ch['channel_name']} - no messages")
                    continue
                ...
                success_count += 1
                print(f"[ok] [{ch['source']}] {ch['channel_name']} - done ({len(messages)} messages, {len(image_urls)} images)")

            except Exception as e:
                print(f"[error] Failed [{ch['source']}] {ch['channel_name']}: {str(e)}")
                fail_count += 1

        print(f"[ok] Summarization complete - {success_count} succeeded, {fail_count} failed")

    except Exception as e:
        print(f"[error] Scheduler job crashed: {str(e)}")
...
def start_scheduler():
    ...
    thread.start()
    print("[ok] Scheduler started - summarization runs every 2 hours")
    return thread