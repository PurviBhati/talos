  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div>
          <div style={{ fontSize: 22, fontWeight: 800, color: C.text, letterSpacing: '-0.02em' }}>{title}</div>
...
function SummariesSection() {
  ...
  function parseExtraction(text: string) {
    ...
    const links = rawLinks === 'NO_ACTION' ? null :
      (rawLinks?.split(',').map(l => l.trim()).filter(l => /^https?:\/\/.+\..+/.test(l)).slice(0, 3).join(', '));
    ...
    <div style={{ padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: C.muted, marginTop: 3 }}>AI-extracted tasks, files & links from last 20 messages per channel</div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={triggerRefresh} disabled={refreshing} style={{ background: `${C.accent}22`, border: `1px solid ${C.accent}44`, color: refreshing ? C.dim : C.accent, padding: '7px 16px', borderRadius: 8, fontSize: 12, cursor: refreshing ? 'not-allowed' : 'pointer' }}>
            {refreshing ? '⏳ Summarizing...' : '✦ Re-summarize All'}
          </button>
          <button onClick={fetchSummaries} disabled={loading} style={{ background: C.card, border: `1px solid ${C.border}`, color: loading ? C.dim : C.muted, padding: '7px 16px', borderRadius: 8, fontSize: 12, cursor: loading ? 'not-allowed' : 'pointer' }}>
            {loading ? '...' : '↻ Refresh'}
          </button>
        </div>
      </div>
...
function ForwardLogSection() {
  ...
  function statusIcon(s: string) {
    if (s === 'delivered') return ICONS.check(16, '#22c55e');
    if (s === 'failed') return ICONS.error(16, '#ef4444');
    return ICONS.link(16, C.dim);
  }
  ...
  function sourceIcon(s: string) {
    if (s === 'whatsapp') return ICONS.whatsapp(16, C.wa);
    if (s === 'slack') return ICONS.slack(16, C.slack);
    if (s === 'teams') return ICONS.teams(16, C.teams);
    return ICONS.link(16, C.dim);
  }
  function destIcon(d: string) {
    if (d === 'teams') return ICONS.teams(16, C.teams);
    if (d === 'slack') return ICONS.slack(16, C.slack);
    if (d === 'whatsapp') return ICONS.whatsapp(16, C.wa);
    return ICONS.link(16, C.dim);
  }
  ...
  return (
    <div style={{ padding: '32px 40px', maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: C.text, letterSpacing: '-0.02em' }}>System Admin</h1>
          <p style={{ fontSize: 14, color: C.muted, marginTop: 4 }}>Real-time health monitoring and service status</p>
        </div>
        <button onClick={fetchHealth} style={{ padding: '10px 20px', background: `${C.accent}15`, border: `1px solid ${C.accent}33`, borderRadius: 10, color: C.accent, fontWeight: 700, cursor: 'pointer', fontSize: 13, outline: 'none' }}>
          Refresh Status
        </button>
      </div>
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 20, padding: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
              ...
              <span style={{ color: C.text, fontWeight: 600 }}>{health?.teams?.activeCount || 0}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
              <span style={{ color: C.muted }}>Last Renewal</span>
              <span style={{ color: C.text, fontWeight: 600 }}>{health?.teams?.lastRenewal ? new Date(health.teams.lastRenewal).toLocaleTimeString() : 'Never'}</span>
            </div>
            <div style={{ fontSize: 11, color: C.dim, background: `${C.bg}80`, padding: '8px 12px', borderRadius: 8, wordBreak: 'break-all', marginTop: 8 }}>
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 20, padding: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
              ...
              <span style={{ color: C.text, fontWeight: 600 }}>{health?.whatsapp?.groups || 0}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
              <span style={{ color: C.muted }}>Last Health Check</span>
              <span style={{ color: C.text, fontWeight: 600 }}>{health?.whatsapp?.lastHealthCheck ? new Date(health.whatsapp.lastHealthCheck).toLocaleTimeString() : 'Never'}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 20, padding: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
              ...
              <span style={{ color: C.text, fontWeight: 600 }}>{health?.config?.port}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
              <span style={{ color: C.muted }}>Monitored Teams Chats</span>
              <span style={{ color: C.text, fontWeight: 600 }}>{health?.config?.monitoredChats}</span>
            </div>
            <div style={{ fontSize: 11, color: C.dim, background: `${C.bg}80`, padding: '8px 12px', borderRadius: 8, wordBreak: 'break-all', marginTop: 8 }}>
...
export default function Dashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', width: '100vw', maxWidth: '100%', background: C.bg, display: 'flex', fontFamily: "'Inter', -apple-system, sans-serif", color: C.text, overflowX: 'hidden' }}>
      <div style={{ width: 264, background: C.sidebar, borderRight: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', flexShrink: 0, position: 'sticky', top: 0, height: '100vh', overflowY: 'auto' }}>
          ...
          {navItems.map(item => {
            ...
            return (
              <button key={item.key} onClick={() => setActiveNav(item.key)}
                style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '13px 20px', border: 'none', cursor: 'pointer', textAlign: 'left', background: active ? C.card : 'transparent', borderLeft: active ? `3px solid ${C.accent}` : '3px solid transparent' }}>
        ...
        <div style={{ padding: '16px 20px', borderTop: `1px solid ${C.border}` }}>
          <button onClick={() => setTheme(t => t === 'dark' ? 'light' : 'dark')}
            style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', background: `${C.accent}10`, border: `1px solid ${C.accent}33`, borderRadius: 12, cursor: 'pointer', color: C.text, transition: 'all 0.2s', outline: 'none' }}
            ...
            <span style={{ fontSize: 13, fontWeight: 700 }}>{theme === 'dark' ? 'Green Light' : 'Deep Dark'}</span>
          </button>
        </div>
      </div>
